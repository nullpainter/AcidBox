...
inline void handleNoteOn(uint8_t inChannel, uint8_t inNote, uint8_t inVelocity) {
  ...
  if (inChannel == DRUM_MIDI_CHAN) {
    Drums.NoteOn(inNote, inVelocity);
  } else if (inChannel == SYNTH1_MIDI_CHAN) {
    Synth1.on_midi_noteON(inNote, inVelocity);
  } else if (inChannel == SYNTH2_MIDI_CHAN) {
    Synth2.on_midi_noteON(inNote, inVelocity);
  }
}

inline void handleNoteOff(uint8_t inChannel, uint8_t inNote, uint8_t inVelocity) {
  if (inChannel == DRUM_MIDI_CHAN) {
    Drums.NoteOff(inNote);
  } else if (inChannel == SYNTH1_MIDI_CHAN) {
    Synth1.on_midi_noteOFF(inNote, inVelocity);
  } else if (inChannel == SYNTH2_MIDI_CHAN) {
    Synth2.on_midi_noteOFF(inNote, inVelocity);
  }
}
...
inline void handleCC(uint8_t inChannel, uint8_t cc_number, uint8_t cc_value) {
  ...
  switch (cc_number) {  // global parameters yet set via ANY channel CCs
    ...
    default:
      if (inChannel == DRUM_MIDI_CHAN) {
        Drums.ParseCC(cc_number, cc_value);
      } else if (inChannel == SYNTH1_MIDI_CHAN) {
        Synth1.ParseCC(cc_number, cc_value);
      } else if (inChannel == SYNTH2_MIDI_CHAN) {
        Synth2.ParseCC(cc_number, cc_value);
      }
  }
}
...
void handleProgramChange(uint8_t inChannel, uint8_t number) {
  ...
  switch (inChannel) {
    ...
    case 2:
      ...
      break;
    default:
      DEBF("Unhandled program change: channel %d, number %d\n", inChannel, number);
      return;
  }

  setButtonState(number, pressed);
}

inline void handlePitchBend(uint8_t inChannel, int number) {
  if (inChannel == DRUM_MIDI_CHAN) {
    Drums.PitchBend(number);
  } else if (inChannel == SYNTH1_MIDI_CHAN) {
    Synth1.PitchBend(number);
  } else if (inChannel == SYNTH2_MIDI_CHAN) {
    Synth2.PitchBend(number);
  }
}
