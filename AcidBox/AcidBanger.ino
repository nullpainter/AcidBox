...
static struct Button buttons[ButLast]; // TODO remove , as well as struct
...
void run_ui() {
  ...
  if (held.source != -1 && held.target != -1) {
    source_memory = held.source - MEMORY_BUTTON_OFFSET;
    uint8_t target_memory = held.target - MEMORY_BUTTON_OFFSET;

    DEBF("> Copy memory %d to %d\r\n", source_memory, target_memory);

    memcpy(&memories[target_memory], &memories[source_memory], sizeof(memories[0]));
  } else {

    uint8_t buttonPressed;
    buttonPressed = getButtonPressed();

    if (buttonPressed == 11) {  // TODO isthis working? Also,how about a separate button to do drum fills

      midiRampsEnabled = !midiRampsEnabled;
      if (midiRampsEnabled) {
        DEBF("> MIDI ramps enabled\n");
      } else {
        DEBF("> MIDI ramps disabled\n");
      }
    }
    else
    ...
    //if (just_pressed(ButPat1)) {
    if (buttonPressed == 0) {
      DEBF("> Regenerate 303 #1 melody from current note set\n");
      mem_generate_melody_and_seed(cur_memory, 0);
      print_memory(cur_memory);
    }
    // if (just_pressed(ButPat1 + 1)) {
    else if (buttonPressed == 1) {
      DEBF("> Regenerate 303 #2 melody from current note set\n");
      mem_generate_melody_and_seed(cur_memory, 1);
      print_memory(cur_memory);
    }
    // if (just_pressed(ButNotes)) {
    else if (buttonPressed == 2) {
      DEBF("> Regenerate note set\n");
      mem_generate_note_set(cur_memory);
      print_memory(cur_memory);
    }
    // if (just_pressed(ButDrums)) {
    else if (buttonPressed == 3) {
      DEBF("> Regenerate 808 pattern\n");
      mem_generate_drums(cur_memory, DrumStraight);
      print_memory(cur_memory);
    }
    ...
    } else if (buttonPressed >= MEMORY_BUTTON_OFFSET && buttonPressed <= MEMORY_BUTTON_OFFSET + NumMemories) {

      cur_memory = buttonPressed - MEMORY_BUTTON_OFFSET;

      DEBF("> Switching to memory %d\n", cur_memory);
    }
  }
}
